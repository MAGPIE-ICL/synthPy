...
# The following functions are methods to be called by the solve()
    ...
    def dndr_test(self, r, x, y, z):
        grad = jnp.zeros_like(r)

        dndx = -0.5 * c ** 2 * jnp.gradient(self.ne_nc / (3.14207787e-4 * self.omega ** 2), x, axis = 0)
        grad = grad.at[0, :].set(self.trilinearInterpolator(x, y, z, self.ScalarDomain.lengths, self.ScalarDomain.dim, dndx, r.T, fill_value = 0.0))
        del dndx

        dndy = -0.5 * c ** 2 * jnp.gradient(self.ne_nc / (3.14207787e-4 * self.omega ** 2), y, axis = 1)
        grad = grad.at[1, :].set(self.trilinearInterpolator(x, y, z, self.ScalarDomain.lengths, self.ScalarDomain.dim, dndy, r.T, fill_value = 0.0))
        del dndy

        dndz = -0.5 * c ** 2 * jnp.gradient(self.ne_nc / (3.14207787e-4 * self.omega ** 2), z, axis = 2)
        grad = grad.at[2, :].set(self.trilinearInterpolator(x, y, z, self.ScalarDomain.lengths, self.ScalarDomain.dim, dndz, r.T, fill_value = 0.0))
        del dndz
    ...
    def solve(self, return_E = False, parallelise = True, jitted = True):
        ...
        if not parallelise:
            ...
            def dsdt_ODE(t, y, args):
                return dsdt(t, y, args[0], args[1], args[2], args[3], args[4]) * norm_factor
            ...
            args = (self, parallelise, x, y, z) # passed args must be hashable to be made static for jax.jit, tuple is hashable, array & dict are not
        ...
        if not parallelise:
            ...
            print("\nWe reshape into the form:", self.Beam.rf.shape)
            #else:
...
def dsdt(t, s, Propagator, parallelise, x, y, z):
    ...
    #sprime = sprime.at[3:6, :].set(Propagator.dndr_test(r, x, y, z))
    sprime = sprime.at[:3, :].set(v)

    sprime = sprime.at[6, :].set(Propagator.atten(r) * a)
    sprime = sprime.at[7, :].set(Propagator.phase(r))
    sprime = sprime.at[8, :].set(Propagator.neB(r, v))
